...
function openComponentModal({ mode = "create", data = {} }) {
  ...
  openModal({
    ...
    `,
    onSubmit: () => {
      const portfolioSelect = $("#portfolioSelect").val();
      ...
      if (isEdit) {
        ...
        payload.payment_date = now;

        const lastId = rawData.length > 0 ? rawData[0].id : null;
        let newId = "INVEST-001";

        if (lastId && /^INVEST-\d+$/.test(lastId)) {
          const lastNumber = parseInt(lastId.split("-")[1], 10);
          const nextNumber = lastNumber + 1;
          newId = `INVEST-${String(nextNumber).padStart(3, "0")}`;
        }
      ...
      fetch(
        ...
        .then(() => {
          ...
          showToast(
            isEdit
              ? "Investation updated successfully"
              : "Investation added successfully"
          );
        })
        .catch((err) => {
