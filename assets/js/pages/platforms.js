document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelector(".new-modal-btn").addEventListener("click", () => {
    ...
    modalEl.querySelector(".modal-title").innerText = "New Platform";
    modalEl.querySelector("#btn-submit-modal").innerText = "Add Platform";
  ...
  document.addEventListener("click", function (e) {
    ...
    const platform = rows.find((row) => row[0] === platformId);
    if (!platform) return;
    ...
    modalEl.querySelector(".modal-title").innerText = "Edit Platform";
    modalEl.querySelector("#btn-submit-modal").innerText = "Edit Platform";
    const form = document.getElementById("modalForm");

    form.querySelector("#name").value = platform[1] || "";

    let idInput = form.querySelector('input[name="platform_id"]');
    if (!idInput) {
      idInput = document.createElement("input");
      idInput.type = "hidden";
      idInput.name = "platform_id";
      form.appendChild(idInput);
    }
    idInput.value = platformId;
  });
  ...
  document.addEventListener("click", function (e) {
    ...
    Swal.fire({
      ...
      fetch(
        ...
        .then((response) => {
          if (response.result === "success") {
            localStorage.setItem("platformSaved", "deleted");
            location.reload();
          } else {
        ...
        .catch((err) => {
          ...
          Swal.fire("Error", "Error deleting platform.", "error");
        });
    });
  });
  ...
  document.getElementById("modalForm").addEventListener("submit", function (e) {
    ...
    let platfromIdInput = form.querySelector('input[name="platform_id"]');
    let platfromId;

    if (platfromIdInput) {
      platfromId = platfromIdInput.value;
    } else {
      let maxId = 0;
      rows.forEach((row) => {
        if (row[0] && row[0].startsWith("PLATFORM-")) {
          const num = parseInt(row[0].split("-")[1], 10);
          if (!isNaN(num) && num > maxId) maxId = num;
        }
      });
      const nextIdNum = maxId + 1;
      platfromId = "PLATFORM-" + String(nextIdNum).padStart(3, "0");
    }

    const now = new Date().toISOString();

    const data = {
      action: platfromIdInput ? "platform_update" : "platform_create",
      platform_id: platfromId,
      name: form.name.value,
      inserted_date: now,
      updated_date: platfromIdInput ? now : now,
    };
  ...
  // ✅ Toast on page load
  const status = localStorage.getItem("platformSaved");
  if (status) {
    const messages = {
      saved: "Platform saved successfully",
      edited: "Platform edited successfully",
      deleted: "Platform deleted successfully",
    };
    showToast("success", messages[status] || "Operation completed");
    localStorage.removeItem("platformSaved");
  }
